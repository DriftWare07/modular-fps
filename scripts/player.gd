...
func _physics_process(delta):
	...
	if Input.is_action_pressed("primary_fire"): weaponhandler.primary_fire()
	
	if Input.is_action_just_released("weapon_toggle_up"):
		weaponhandler.toggle(1)
	elif Input.is_action_just_released("weapon_toggle_down"):
		weaponhandler.toggle(-1)
	...
	#$AnimationTree.set("parameters/walkblend/blend_position", input_dir)
