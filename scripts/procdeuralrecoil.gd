...
func _physics_process(delta):
	...
	if current_time < 1:
		...
		target_object.rotation.z = lerp(def_rot.z, def_rot.z + target_rot.z, lerp_speed * delta)
		target_object.rotation.x = lerp(def_rot.x, def_rot.x + target_rot.x, lerp_speed * delta)
